        ...
        try {
            DB::transaction(function () {
                ...
                if (!empty($this->patologias)) {
                    $paciente->patologias()->attach($this->patologias, ['fecha_desde' => $ahora]);
                }

                if (!empty($this->sintomas)) {
                    $paciente->sintomas()->attach($this->sintomas, ['fecha_desde' => $ahora]);
                }
            });
                ...
                <div class="space-y-4">
                    @foreach ($this->todasPatologias as $pat)
                        <div class="flex items-center gap-2">
                            <input id="patologia-{{ $pat->id }}" type="checkbox" class="checkbox-formulario" value="{{ $pat->id }}" wire:model="patologias">
